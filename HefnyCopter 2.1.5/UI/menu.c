...
uint8_t doMenu(menu_t *menu)
{
	...
	if (menu->top > 0)
	{
		LCD_WriteChar('U');// lcdWriteGlyph_P(&glyArrowUp, 0);
	}
	...
	{
		LCD_WriteChar('D'); //lcdWriteGlyph_P(&glyArrowDown, 0);
	}	
	return 0;
}
...
void _hHomePage()
{
	...
	else if (nFlyingModes == FLYINGMODE_GTUNE)
	{
		LCD_WriteString_P(PSTR("G-TUNE"));
	}
...
void _hSensorTest()
{
	...
	{
		int16_t temp = Sensors_Latest[i];
		LCD_WriteValue(i,48,temp,4,(GyroLowLimit > temp) || (temp > GyroHighLimit));
	}
	 LCD_WriteValue(6,48,ADCPort_Get(V_BAT_PNUM),4,false);
...
//#ifdef DEBUG_ME
	...
	for (int i=0; i<SENSORS_IMU;++i)
	{
		LCD_WriteValue_double_ex(i,48,Sensors_Latest[i],10,false);
	}
		//YAWAngle += OldAngle ;
		...
		LCD_WriteValue_double_ex(4,12,CompGyroPitch,9,false);
		LCD_WriteValue_double_ex (1,12,Config.GyroParams[0]._P,10,false);
		LCD_WriteValue_double_ex (2,12,Config.GyroParams[0]._I,10,false);
