...
#ifdef KK21
 uint8_t OldPortAValue;

__attribute__ ((section(".lowtext")))
ISR(RX2_PITC_YAW_vect)
{
	uint8_t Changes = PINA ^ OldPortAValue;  // capture changed bit
	OldPortAValue = PINA;
	uint16_t TCNT1_TEMP = TCNT1;
...
void RX_Init(void)
{
	...
	*/
#if defined (KK21)
	PCICR  |= _BV(PCIE0)   |_BV(PCIE1)   | _BV(PCIE2);											// enable PCI1 and PCI2
	PCMSK0 |= _BV(PCINT4)  | _BV(PCINT5);														// enable A5,A4 RUB , ELE
	PCMSK1 |= _BV(PCINT8);																		// enable PCINT8 (AUX) -> PCI1
	PCMSK2 |= _BV(PCINT21) | _BV(PCINT23);														// THR - AIL
	PCIFR  |= _BV(PCIF0)   |_BV(PCIF1)   | _BV(PCIF2);														// clear interrupts
#else
